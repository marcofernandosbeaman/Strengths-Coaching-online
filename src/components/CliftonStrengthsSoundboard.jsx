...
export default function CliftonStrengthsSoundboard() {
  ...
  useEffect(() => {
    try {
      ...
      console.assert(
        keys.length === 34,
        `Expected 34 strengths, found ${keys.length}`
      );
      console.assert(
        new Set(keys).size === keys.length,
        "Duplicate theme keys found"
      );
      console.assert(
        keys.every((k) => k in COMMUNICATION_STYLES),
        "Missing COMMUNICATION_STYLES entries"
      );
      console.assert(
        keys.every((k) => k in BALCONY_BASEMENT),
        "Missing BALCONY_BASEMENT entries"
      );
      console.assert(selected.length <= 5, "Selected more than 5 strengths");
      console.assert(
        keys.every((k) => getStrengthColor(k) !== "#999"),
        "Missing domain colour mapping for some keys"
      );
    } catch {}
  }, []);
  ...
  const setLevel = (key, v) =>
    setLevels((prev) => ({ ...prev, [key]: clamp(v) }));
    ...
    input.onchange = async () => {
      ...
      try {
        ...
        if (Array.isArray(data.selected) && typeof data.levels === "object") {
          const safeSelected = data.selected
            .filter((k) => STRENGTHS.some((s) => s.key === k))
            .slice(0, 5);
      ...
      <div className="mx-auto max-w-7xl p-6 md:p-10">
        ...
        <header className="mb-6">
          ...
          <p className="text-neutral-600 mt-2 max-w-2xl">
            Select up to five strengths, then mix the levels like a live sound
            desk. Click a track or drag the coloured label to set the level.
            Arrow keys work for fine control.
          </p>
        </header>
                ...
                <Card className="rounded-2xl border-neutral-200 max-w-xl">
                  <CardContent className="p-3">
                    <div className="text-xs font-semibold mb-1">
                      Combined investment focus for your current mix
                    </div>
                    <pre className="whitespace-pre-wrap text-xs text-neutral-700">
                      {generateCombinedInvestment(selected)}
                    </pre>
                  </CardContent>
                </Card>
              )}
                ...
                <div>
                  <details className="relative overflow-hidden rounded-xl border border-neutral-200 watermark">
                    ...
                    <div className="relative z-10 p-3 text-sm text-neutral-700 space-y-2 bg-white/70">
                      <p>
                        The way you prefer to communicate is not random, it is
                        strongly influenced by your CliftonStrengths.
                      </p>
                      <p>
                        Each of your themes brings its own natural style. Some
                        people like conversations to be logical and structured,
                        others prefer emotional connection or big ideas, and
                        some want to get straight to action.
                      </p>
                      <p>
                        None of these are right or wrong, they are simply
                        different defaults.
                      </p>
                      <p className="mt-2">
                        In this activity, you will explore how your strengths
                        show up in communication, what helps you express
                        yourself at your best, and what makes it harder. The
                        goal is not to fix anything, it is to understand your
                        natural style so you can communicate with more
                        confidence, and let others know how to get the best from
                        you.
                      </p>
                    </div>
                  </details>
                        ...
                        >
                          <CardContent className="py-4 relative">
                            <div
                              ...
                              style={{
                                background:
                                  "radial-gradient(circle at 50% 15%, rgba(255,255,255,0.55), rgba(255,255,255,0) 65%)",
                              }}
                            />
                            ...
                            <p className="text-sm opacity-95 mt-1">
                              {COMMUNICATION_STYLES[s.key] ??
                                "No communication style available."}
                            </p>
                            <div className="mt-3 text-xs text-white/90">
                              <ul className="list-disc list-inside space-y-1">
                                <li>
                                  When do you communicate best with this theme?
                                </li>
                                <li>
                                  What might others need from you to connect
                                  well?
                                </li>
                                <li>
                                  How could you flex your style for different
                                  settings?
                                </li>
                              </ul>
                            </div>
                    ...
                    <>
                      <details className="transition-all duration-300 overflow-hidden bg-neutral-100 border border-neutral-200 rounded-xl p-3 text-sm text-neutral-700">
                        <summary className="cursor-pointer font-semibold">
                          🔍 What it Means & 🎯 Why It Matters (click to expand)
                        </summary>
                        <ul className="list-disc list-inside mt-2 mb-2">
                          <li>
                            <strong>Balcony</strong> - Mature, helpful,
                            energising expression of a strength.
                            <br />
                            <em>
                              Empathy (Balcony): You sense how others feel and
                              help them feel understood.
                            </em>
                          </li>
                          <li>
                            <strong>Basement</strong> - Overused or reactive
                            version of that same strength.
                            <br />
                            <em>
                              Empathy (Basement): You absorb everyone's feelings
                              and get emotionally drained.
                            </em>
                          </li>
                        </ul>
                        ...
                        <ul className="list-disc list-inside mt-1 mb-2">
                          ...
                          <li>
                            <strong>Context</strong> - Is it the right strength
                            in this moment?
                          </li>
                          <li>
                            <strong>Maturity</strong> - Are you using it
                            intentionally or on autopilot?
                          </li>
                        </ul>
                        <strong>
                          The goal is not to fix your basements. It is to:
                        </strong>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-2 border-t border-white/30 pt-3">
                              <div className="bg-black/20 rounded-xl p-3 order-1">
                                ...
                                <p className="text-sm opacity-95">
                                  {BALCONY_BASEMENT[s.key]?.basement ?? "—"}
                                </p>
                              </div>
                              <div className="bg-white/10 rounded-xl p-3 order-2">
                                <div className="text-xs font-semibold bg-yellow-200/20 text-yellow-800 rounded px-1">
                                  ☀️ Balcony{" "}
                                  <span className="italic">(At best)</span>
                                </div>
                                <p className="text-sm opacity-95">
                                  {BALCONY_BASEMENT[s.key]?.balcony ?? "—"}
                                </p>
                              </div>
                            </div>
                            <div className="mt-3 text-xs text-white/90">
                              <ul className="list-disc list-inside space-y-1">
                                <li>
                                  When does this strength serve you most, and
                                  when does it get in your way?
                                </li>
                                <li>
                                  What signals tell you when you are edging into
                                  the basement?
                                </li>
                                <li>
                                  Who helps you stay in the balcony version of
                                  this strength?
                                </li>
                                <li>
                                  No judgement, just awareness. Where are you
                                  today?
                                </li>
                              </ul>
                            </div>
